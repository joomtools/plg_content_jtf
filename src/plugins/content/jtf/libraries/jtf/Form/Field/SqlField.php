...
class SQLField extends ListField
{
	...
	protected function processQuery($conditions, $filters, $defaults)
	{
		...
		if (is_array($filters))
		{
			$html_filters = $this->app->getUserStateFromRequest($this->context . '.filter', 'filter', array(), 'array');

			foreach ($filters as $k => $value)
			{
				if (!empty($html_filters[$value]))
				{
					$escape = $this->db->quote($this->db->escape($html_filters[$value]), false);

					$query->where("{$value} = {$escape}");
				}
				elseif (!empty($defaults[$value]))
				{
					$escape = $this->db->quote($this->db->escape($defaults[$value]), false);
