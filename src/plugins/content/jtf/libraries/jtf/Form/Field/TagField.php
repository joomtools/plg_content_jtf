...
class TagField extends ListField
{
	...
	protected function getOptions()
	{
		...
		$query = $this->db->getQuery(true)
			...
			->from('#__tags AS a')
			->join('LEFT', $this->db->qn('#__tags') . ' AS b ON a.lft > b.lft AND a.rgt < b.rgt');
		...
		if ($this->app->isClient('site') && Multilanguage::isEnabled())
		{
			...
			{
				$query->where('a.language in (' . $this->db->quote($tag) . ',' . $this->db->quote('*') . ')');
			}
		}
		// Filter language
		elseif (!empty($this->element['language']))
		{
			...
			{
				$language = implode(',', $this->db->quote(explode(',', $this->element['language'])));
			}
			else
			{
				$language = $this->db->quote($this->element['language']);
			}

			$query->where($this->db->quoteName('a.language') . ' IN (' . $language . ')');
		}

		$query->where($this->db->qn('a.lft') . ' > 0');
